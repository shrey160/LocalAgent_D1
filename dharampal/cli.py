...
def _load_model(lms):
    print(f"Loading model {MODEL_ID} as '{IDENTIFIER}' (may take a while on first run)...")
...
def start():
    ...
    _save_state(ui_pid=ui_pid, identifier=IDENTIFIER)
    print(f"Chat window launched (PID {ui_pid}).")
    print(f"UI log: {UI_LOG_FILE}  (check this file if no window appears)")
